...
Nimiq 2.0 has four types of accounts: basic accounts, HTLC accounts, vesting accounts and staking accounts. Users can operate with their accounts and interact with the blockchain through [transactions](/docs/transactions). Each account has a unique address. They are stored in the state tree of the blockchain.
...
For a detailed explanation about the staking contract, follow this [link](/docs/staking-contract) to read on validators, stakers, and their interactions with the staking contract.
