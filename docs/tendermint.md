...
Albatross implements Tendermint to produce macro blocks, both checkpoint and election. Validators agree on data such as the [punishments](/just-the-docs/docs/punishments) sets and the list of validators that will receive the [rewards](/just-the-docs/docs/rewards) from the previous batch.

Elected validators use their view [slots](/just-the-docs/docs/slots) to propose a macro block and their validator slots to sign the block proposal. Each one is selected from the view slot list and validator slot list, respectively. Elected validators with their view slots disabled can't propose macro blocks but can still use their validator slots to sign the proposals.
